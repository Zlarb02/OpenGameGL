...
export function FollowCamera({ target }: FollowCameraProps) {
  ...
  useEffect(() => {
    ...
    const onPointerMove = (e: PointerEvent) => {
      ...
      spherical.current.theta -= deltaX * options.mouseSensitivity;

      // Apply Y-axis inversion based on settings
      const yMultiplier = options.invertY ? 1 : -1;
      spherical.current.phi += deltaY * options.mouseSensitivity * yMultiplier;

      // Clamp vertical rotation
      // Note: phi=0 is looking UP, phi=180 is looking DOWN in spherical coords
      const minPhi = MathUtils.degToRad(controls.minPolarAngle);
      const maxPhi = MathUtils.degToRad(controls.maxPolarAngle);
    ...
    const onWheel = (e: WheelEvent) => {
      ...
      // Zoom in/out
      spherical.current.radius += e.deltaY * 0.01 * controls.zoomSpeed;
  ...
  }, [gl, controls.mouseSensitivity, controls.invertY, controls.minPolarAngle, controls.maxPolarAngle, currentMinDistance, controls.maxDistance, controls.zoomSpeed]);
  ...
  useFrame(() => {
    ...
    // En mode rifle : rotation quasi-instantanée pour caméra rigide
    const rotationSmoothingFactor = weaponEquipped ? 0.9 : controls.rotationSmoothing;
    ...
    // En mode rifle : caméra très rigide pour éliminer tout pivotement
    const smoothingFactor = weaponEquipped ? 0.7 : controls.positionSmoothing;
    currentCameraPos.current.lerp(desiredCameraPos.current, smoothingFactor);
