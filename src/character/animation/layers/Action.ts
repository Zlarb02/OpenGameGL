...
/**
 * Action - Manages action animations (shoot, reload, etc.)
 */
export class Action implements IAnimationLayer {
  ...
  async initialize(clips: AnimationClip[]): Promise<void> {
    console.log('[Action] Initializing...');

    // Load action animations
    await this.loadActionAnimations();

    console.log('[Action] Initialized with states:', Array.from(this.states.keys()));
  }
    ...
    const loadAnim = async (path: string, stateName: string, duration?: number) => {
      try {
        ...
        if (clip) {
          ...
          console.log(`[Action] Loaded ${stateName} (duration: ${clip.duration.toFixed(2)}s)`);
        }
      } catch (error) {
        console.warn(`[Action] Failed to load ${stateName}:`, error);
      }
    };
  ...
  playAction(actionName: string): number {
    ...
    if (!state) {
      console.warn(`[Action] Action ${actionName} not found`);
      return 0;
    }
    ...
    console.log(`[Action] Playing action: ${actionName} (${this.actionDuration}ms)`);
  ...
  update(deltaTime: number, context: AnimationContext): string | null {
    ...
    if (this.currentAction) {
      if (!this.isActionPlaying()) {
        // Action finished
        console.log(`[Action] Action ${this.currentAction} finished`);
